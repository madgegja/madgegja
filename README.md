...
<p align="center">
  <b>Toomics Global BizOps 산출물 포털</b> — 운영 매뉴얼 · 분석 리포트 · 소명 템플릿 · AI 챗봇 기획 · 대시보드까지<br>
  BizOps 팀이 생산한 <b>29개 문서</b>를 한곳에서 열람할 수 있는 GitHub Pages 기반 공개 포털
</p>

```
29개 문서 — 5개 카테고리 — 10개 글로벌 마켓

📞 CS 운영 (10)           💳 차지백 (3)           💬 AI 챗봇 (13)
 ├─ 매뉴얼 EN/KO v2.1     ├─ 운영 매뉴얼 v1.3     ├─ 기획·개요 (5)
 ├─ 반복문의 대응 매뉴얼    ├─ 소명 폼 V5           │  ├─ 비용 분석
 ├─ 분석·시각화 (3)        └─ 증빙 템플릿 v2        │  ├─ One Pager
 │  ├─ Fraud 분석 리포트                            │  ├─ 프로젝트 로드맵
 │  ├─ CS 인포그래픽       🛡️ IP 보호 (1)           │  ├─ 아키텍처 설명서
 │  └─ 티켓 레벨 분석       └─ IP 보호 운영 매뉴얼    │  └─ 운영 인포그래픽
 ├─ 프로세스 플로우 (3)                              ├─ 운영·교육 (5)
 │  ├─ CS 플로우            📊 대시보드 (2)          │  ├─ 운영자 가이드 / 시연 시나리오
 │  ├─ CB 플로우            ├─ Ticket Dashboard     │  ├─ 운영 매뉴얼 / 교육 자료
 │  └─ 자동화 로드맵         └─ 대시보드 매뉴얼       │  └─ 테스트 시나리오
 └─ 주간 리포트 아카이브                              └─ 플로우·에스컬레이션 (3)
                                                       ├─ 에스컬레이션 시나리오
                                                       ├─ 파이프라인 플로우
                                                       └─ 처리 흐름도
```
...
**주요 특징**:
- 📄 **이중 언어 매뉴얼** — CS 운영 매뉴얼 EN/KO 동시 제공, 복사 즉시 사용 가능한 응대 템플릿 내장
- 📊 **데이터 기반 분석물** — 11,621건 × 7개월 실데이터 분석 → 인포그래픽 · Fraud 리포트 · 티켓 레벨 분석
- 💳 **차지백 소명 체계** — 8개 PG사 대응 매뉴얼 + 증빙 폼 V5 + 증빙 템플릿 v2
- 💬 **AI 챗봇 기획 풀세트** — 아키텍처 → 로드맵 → 시연 시나리오 → 교육 자료 → 운영 매뉴얼 (13종)
- 🖥️ **인터랙티브 대시보드** — CS Ticket Dashboard + 운영 매뉴얼 (실시간 시각화)
- 🛡️ **IP 보호 프로세스** — DMCA 신고 체계 통합 운영 매뉴얼

**포털 기술**: GitHub Pages · HTML/CSS · Claude AI 자동 생성 · `@media print` PDF 변환 지원
